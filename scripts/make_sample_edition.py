...
from sixth_estate.build import (assemble_edition, build_money_box,  # noqa: E402
                                build_sports_box, build_this_day)
...
def quick_hits():
    return [
        ...
        QuickHit(lane="Economy",
                 text="DEMO: Weekly jobless claims fell slightly below forecast in this illustrative example from fictional data.",
                 source=S("https://www.dol.gov/ui/data.pdf", "Dept. of Labor")),
    ]
...
def data_boxes():
    ...
    sports = build_sports_box(
        scores=[
            DataMetric("NYY 5, BOS 3 (demo)", "Final", S("https://www.espn.com/mlb/scoreboard", "ESPN (MLB)"), "2026-07-19"),
            DataMetric("LAL 108, GSW 101 (demo)", "Final", S("https://www.espn.com/nba/scoreboard", "ESPN (NBA)"), "2026-07-19"),
        ],
    )
    return [money, sports]
...
def receipt():
    return Receipt(
        title="DEMO: Federal Register — Data-Broker Disclosure Rule",
        description="Final rule requiring data brokers to disclose categories of personal information sold and recipients. 12-month compliance window.",
        source=S("https://www.federalregister.gov/documents/2026/07/18/2026-15321/data-broker-disclosure-rule", "Federal Register"),
    )
