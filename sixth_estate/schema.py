...
@dataclass
class Receipt:
    """A primary-source document (filing, order, data release). One per edition
    max, free-access only. The point is 'here's the proof, go read it.'"""
    title: str
    description: str          # one-line description of what the document is
    source: Optional[Source] = None
...
class Edition:
    ...
    def from_dict(cls, d: dict) -> "Edition":
        ...
        return cls(
            ...
            receipt=Receipt.from_dict(receipt) if receipt else None,
            meta=d.get("meta", {}),
        )
    ...
    def all_sources(self) -> list[Source]:
        ...
        if self.receipt and self.receipt.source:
            out.append(self.receipt.source)
        return out
