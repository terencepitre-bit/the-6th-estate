...
def cmd_curate(args) -> int:
    """Assemble a draft edition. Without credentials this produces an empty
    scaffold the operator fills; with credentials it runs the full live pipeline:
    RSS discovery → Gemini writing → data feeds → assembly. Never fabricates."""
    ...
    # If Gemini key is set, run the full live pipeline.
    # Otherwise, produce an empty scaffold the operator fills.
    if config.GEMINI_API_KEY:
        from .pipeline import run_pipeline
        print(f"Running live pipeline for {date}...")
        ...
        print(f"Pipeline complete: {total}/{config.TOTAL_ITEMS} items.")
    else:
        ed = Edition(date=date, meta={"brand": config.BRAND,
                                      "generated_at": datetime.now(timezone.utc).isoformat(),
                                      "note": "scaffold — no GEMINI_API_KEY; populate manually"})
        total = 0
        print(f"No GEMINI_API_KEY; created empty scaffold for {date}.")
