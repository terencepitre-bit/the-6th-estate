...
# ── Quick hit quality filters ────────────────────────────────────────────────
# Candidates must score >= this to be eligible as a quick hit.
QUICK_HIT_MIN_INTEREST_SCORE = 2  # 1-5 scale; 2 filters out the dullest stories
...
DEFAULT_RSS_FEEDS = [
    ...
    # ── Education ──
    "https://feeds.feedburner.com/hechingerreport",         # Hechinger Report (ed)
    "https://www.insidehighered.com/rss/feed",              # Inside Higher Ed
    # ── Science / tech / health ──
    "https://rss.nytimes.com/services/xml/rss/nyt/Science.xml",   # verify permitted
    "https://feeds.arstechnica.com/arstechnica/index",            # verify permitted
    "https://feeds.npr.org/1007/rss.xml",                         # NPR science
    "https://theconversation.com/us/articles.atom",               # The Conversation
    # ── Culture / arts ──
    "https://apnews.com/hub/entertainment?output=rss",      # verify permitted
    "https://feeds.npr.org/1008/rss.xml",                   # NPR arts & life
    # ── Real estate / housing ──
    "https://apnews.com/hub/real-estate?output=rss",        # verify permitted
    # ── Sports (general — separate from Sports Box RSS) ──
    "https://apnews.com/hub/sports?output=rss",             # verify permitted
    # ── Personal excellence / human interest ──
    "https://feeds.npr.org/1003/rss.xml",                   # NPR national (human interest)
    "https://apnews.com/hub/oddities?output=rss",           # AP oddities (unusual stories)
]
...
# Sports news RSS feeds — used to pull 1-2 editorial quick hits for the Sports Box.
# These are separate from score data. Configure via SIXTHE_SPORTS_RSS_FEEDS.
DEFAULT_SPORTS_RSS_FEEDS = [
    "https://www.espn.com/espn/rss/news",
    "https://apnews.com/hub/sports?output=rss",
]

def _sports_rss_feeds() -> list[str]:
    raw = _env("SIXTHE_SPORTS_RSS_FEEDS")
    if raw:
        return [u.strip() for u in raw.split(",") if u.strip()]
    return list(DEFAULT_SPORTS_RSS_FEEDS)

SPORTS_QUICK_HITS_MAX = 2  # Max editorial quick hits in the Sports Box
