...
def _build_quick_hits(writer: ClaudeWriter, candidates: list[Candidate],
    ...
    Uses round-robin across lanes to ensure diversity: picks one candidate from
    each populated lane before taking a second from any lane. This prevents
    government-heavy feeds from dominating the quick hits.
    """
    remaining = [c for c in candidates if c.url not in used_urls]

    # Bucket remaining candidates by lane for round-robin selection
    lane_pools: dict[int, list[Candidate]] = {}
    for c in remaining:
        lane = _classify_lane(c)
        lane_pools.setdefault(lane, []).append(c)

    # Shuffle within each lane for variety
    for pool in lane_pools.values():
        random.shuffle(pool)
