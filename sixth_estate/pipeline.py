...
from .schema import (Briefing, Closer, DataMetric, Edition, QuickHit, Receipt,
                     Source, VoiceBlock)
from .writer.gemini import GeminiWriter, WriterBudgetExceeded, WriterDisabled
...
def _build_briefings(writer: GeminiWriter, candidates: list[Candidate],
...
def _build_quick_hits(writer: GeminiWriter, candidates: list[Candidate],
...
def run_pipeline(edition_date: Optional[str] = None,
    ...
    writer = GeminiWriter(logger=log)
    print(f"  Gemini writer: model={config.GEMINI_MODEL_BRIEFINGS}, "
    ...
    try:
        ...
        briefings = []
    print(f"  Wrote {len(briefings)} briefings ({writer.calls_used} Gemini calls used).")
    ...
    try:
        ...
        quick_hits = []
    print(f"  Wrote {len(quick_hits)} quick hits ({writer.calls_used} Gemini calls used).")
