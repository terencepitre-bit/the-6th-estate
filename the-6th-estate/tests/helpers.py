...
from sixth_estate.build import (assemble_edition, build_money_box,
                                ...
                                build_this_day)
from sixth_estate.schema import (Briefing, Closer, DataMetric, QuickHit, Source,
                                 VoiceBlock)
...
def valid_data_boxes():
    ...
    board = build_scoreboard_weather_box(
        weather=[DataMetric("Today", "88F Sunny", S("https://api.weather.gov/gridpoints/LWX/96,70/forecast"), "2026-07-20")],
    )
    return [money, board]
...
def valid_voice_blocks():
    ...
    number = build_the_number(DataMetric("30-yr", "6.6%",
                                         S("https://fred.stlouisfed.org/series/MORTGAGE30US"), "2026-07-17"))
    return [this_day, number]
