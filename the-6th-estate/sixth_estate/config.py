...
# ── Canonical edition structure: 4 + 5 + 2 + 2 + 1 = 14 items ────────────────
# These counts are LOCKED. The section-count validator enforces them exactly for
# automatic publication. See docs/THE_6TH_ESTATE_MASTER_MANUAL.md.
N_BRIEFINGS = 4
N_QUICK_HITS = 5
N_DATA_BOXES = 2
N_VOICE_BLOCKS = 2
N_CLOSERS = 1
TOTAL_ITEMS = N_BRIEFINGS + N_QUICK_HITS + N_DATA_BOXES + N_VOICE_BLOCKS + N_CLOSERS  # 14
...
# ── Publication floor ─────────────────────────────────────────────────────────
# Automatic publication requires all 14 validated items. Manual preview may
# approve a reduced edition only if every displayed item individually validates.
PUBLICATION_FLOOR = _env_int("SIXTHE_PUBLICATION_FLOOR", TOTAL_ITEMS)
...
# Sports: provider abstraction. ESPN endpoints must be checked for permitted use;
# prefer official league feeds. Configure the base explicitly to opt in.
SPORTS_PROVIDER = _env("SIXTHE_SPORTS_PROVIDER", "")  # "", "espn", or "league"
...
# changes. Defaults name a Flash-Lite-class model for Quick Hits and a
# Flash-class model for Briefings. VERIFY current model availability/pricing with
# the provider — these are planning defaults, not guarantees.
GEMINI_API_KEY = _env("GEMINI_API_KEY")  # never logged
...
# Hard ceiling on model calls per edition. One batched pass + at most one fallback.
MODEL_CALL_LIMIT = _env_int("SIXTHE_MODEL_CALL_LIMIT", 2)
