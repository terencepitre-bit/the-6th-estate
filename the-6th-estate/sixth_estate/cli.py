...
def cmd_curate(args) -> int:
    ...
    scaffold the operator fills; with credentials + flags it would run discovery,
    data adapters, and the bounded writer. It never fabricates content."""
    ...
    if existing and not args.force:
        ...
        return 1
    # Scaffold-only in this environment (no live credentials). The full pipeline
    # would populate briefings/quick hits/data/voice/closer here.
    ed = Edition(date=date, meta={"brand": config.BRAND,
                                  "generated_at": datetime.now(timezone.utc).isoformat(),
                                  "note": "scaffold — populate via live pipeline or fixture"})
    ed.save(config.EDITIONS_DIR / f"{date}.json")
    st = EditionState.load_or_new(date)
    st.status = CURATED
    st.save()
    log.info("curated", date=date, items=0)
    print(f"Curated draft scaffold for {date}. Populate, then `preview`.")
    return 0
