...
def _landing_page(latest: Edition | None) -> str:
    ...
    <section class="lp-hero">
      ...
      <p class="lp-subhead">{T.esc(config.TAGLINE)} A neutral daily briefing: four briefings, five quick hits, two data boxes, two voice blocks, one closer — every claim sourced.</p>
      {latest_link}
    </section>
    <section class="lp-explain">
      <h2>What you get every morning</h2>
      <ol class="lp-structure">
        <li><strong>Four Briefings</strong> — 60-75 words, exact source links.</li>
        <li><strong>Five Quick Hits</strong> — 25 words max, one free-access source each.</li>
        <li><strong>Two Data Boxes</strong> — money + scoreboard/weather, every value sourced and timestamped.</li>
        <li><strong>Two Voice Blocks</strong> — This Day, The Number.</li>
        <li><strong>One Closer</strong> — a quote, curiosity, or et-cetera.</li>
      </ol>
    </section>
    {_signup_form()}
"""
...
def _signup_form(prefix: str = "") -> str:
    # Honeypot: hidden inline as well as via .lp-hp so a missing stylesheet can
    # never surface it. aria-hidden + tabindex=-1 keep it out of the a11y tree.
    return f"""
    <section class="lp-signup" id="subscribe">
      <h2>Join the daily readers</h2>
      <form class="lp-form" action="{prefix}api/subscribe" method="POST">
        <input type="text" name="website" class="lp-hp" tabindex="-1" autocomplete="off" aria-hidden="true" style="position:absolute!important;left:-9999px!important;width:1px;height:1px;overflow:hidden">
        <input type="email" name="e" placeholder="you@example.com" required aria-label="Email address">
        <input type="text" name="n" placeholder="First name (optional)" aria-label="First name">
...
def _manifesto_page() -> str:
    ...
    <section class="prose">
      ...
      <p>Every edition is the same shape: four briefings, five quick hits, two data
      boxes, two voice blocks, one closer. Every claim carries an exact, direct source
      link — never a homepage, section page, or search result. We prefer free-access
      reader links and never present a paywalled destination as free.</p>
...
def _subscribe_page() -> str:
    ...
    <section class="prose">
      <h1>Subscribe</h1>
      <p>Get {T.esc(config.BRAND)} in your inbox every morning. Free.</p>
      {_signup_form()}
    </section>
"""
