...
from .. import config
from ..schema import Briefing, Closer, DataBox, Edition, QuickHit, Source, VoiceBlock
...
def edition_body(ed: Edition) -> str:
    ...
    <article class="edition">
      ...
      <div class="edition-masthead">
        ...
        <p class="edition-structure">Four Briefings · Five Quick Hits · Two Data Boxes · Two Voice Blocks · One Closer</p>
      </div>

      <section class="sec sec-briefings">
        {section_rail(1, "Briefings", f"{len(ed.briefings)} of {config.N_BRIEFINGS}")}
        {briefings}
      </section>

      <section class="sec sec-quick-hits">
        {section_rail(2, "Quick Hits", f"{len(ed.quick_hits)} of {config.N_QUICK_HITS}")}
        <ul class="quick-hits">{quick_hits}</ul>
      </section>

      <section class="sec sec-data">
        {section_rail(3, "Data Boxes", f"{len(ed.data_boxes)} of {config.N_DATA_BOXES}")}
        <div class="data-grid">{data_boxes}</div>
      </section>

      <section class="sec sec-voice">
        {section_rail(4, "Voice Blocks", f"{len(ed.voice_blocks)} of {config.N_VOICE_BLOCKS}")}
        <div class="voice-grid">{voice_blocks}</div>
      </section>

      <section class="sec sec-closer">
        {section_rail(5, "The Closer", "1 of 1")}
        {closer}
      </section>
    </article>
"""
