...
def test_sports_disabled_without_provider():
    import sixth_estate.config as cfg
    old = cfg.SPORTS_PROVIDER
    try:
        cfg.SPORTS_PROVIDER = ""
        fetch_scores(leagues=["mlb"])
        assert False
    except SportsDisabled:
        pass
    finally:
        cfg.SPORTS_PROVIDER = old


def test_sports_with_transport():
    def transport(league):
        return {"events": [{"name": "Home vs Away",
                "competitions": [{"competitors": [
                    {"team": {"abbreviation": "HOM"}, "score": "5", "winner": True},
                    {"team": {"abbreviation": "AWY"}, "score": "3", "winner": False}]}],
                "status": {"type": {"state": "post", "shortDetail": "Final"}},
                "links": [{"text": "Gamecast", "href": "https://www.espn.com/mlb/game?gameId=1"}],
                "date": "2026-07-19T23:00Z"}]}
    out = fetch_scores(leagues=["mlb"], transport=transport)
    assert len(out) >= 1
    assert "Final" in out[0].value
